...
describe('Diff stylish tests', () => {
  ...
  test('#5 diff plain two json files', () => {
    const stringifyDiff = genDiff(path1Json, path2Json, 'plain');
    expect(stringifyDiff).toEqual(plainOutput);
  });

  test('#6 diff plain two yaml files', () => {
    const stringifyDiff = genDiff(path1Yml, path2Yml, 'plain');
    expect(stringifyDiff).toEqual(plainOutput);
  });

  test('#7 diff plain yaml and yml files', () => {
    const stringifyDiff = genDiff(path1Yaml, path2Yml, 'plain');
    expect(stringifyDiff).toEqual(plainOutput);
  });

  test('#8 diff plain json and yml', () => {
    const stringifyDiff = genDiff(path1Json, path2Yaml, 'plain');
    expect(stringifyDiff).toEqual(plainOutput);
  });
